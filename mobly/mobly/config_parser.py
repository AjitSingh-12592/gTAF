...
def _validate_testbed_configs(testbed_configs):
    ...
    """
    print("In _validate_testbed_configs. ")
    seen_names = set()
    ...
    for config in testbed_configs:
        ...
        device_serial = config['Controllers']
        global dev_li

        print("Test bed Name [config_parser.py] : ", name)
        for i in range(0, len(device_serial['AndroidDevice'])):
            id = device_serial['AndroidDevice'][i]['serial']
            dev_li.append(id)
            print("Device serial number [config_parser.py]: ", id)
