...
class Contacts(base_test.BaseTestClass):
    ...
    def test_contact_permission_grant_from_gmail(self):
        ...
        try:
            ...
            assert self.dut.mbs.waitUntillDisplayElementWithContentDesc(
                _compose) is True, "Compose not found"
            self.dut.mbs.waitAndClickUsingContentDesc(_compose)
    ...
    def test_strong_match_for_duplicates(self):
        ...
        try:
            ...
            assert self.dut.mbs.assertTextDisplayed(
                _merge_duplicate_msg), "Merge duplicates pop up not found"
    ...
    def test_suggestion_welcome_card(self):
        ...
        try:
            ...
            logging.info("Click on Suggestions")
            assert self.dut.mbs.waitUntillDisplayElementWithText(
                "Suggestions") is True, "Suggestions not displayed"
            self.dut.mbs.clickOnText("Suggestions")
            ...
            assert self.dut.mbs.assertTextNotDisplayed(
                _got_it_text) is True, "Got it not dismissed"
    ...
    def test_merge_duplicates(self):
        ...
        try:
            ...
            assert self.dut.mbs.assertTextDisplayed(
                _duplicates[0]) is True, "Duplicates are not shown."
            logging.info("asserted")
            logging.info("assert '%s' ", _duplicates[1])
            assert self.dut.mbs.assertTextDisplayed(
                _duplicates[1]) is True, "Duplicates are not shown."

            self.dut.mbs.clickOnText("Merge")
            time.sleep(2)

            # Verification 3: The duplicates were merged.
            assert self.verify_text_on_screen(
                image=merge_verify_image, txt=merge_text) is True, "Duplicates didn't merge."
    ...
    def test_google_play_service_permission_revoke(self):
        ...
        try:
            ...
            assert self.dut.mbs.waitUntillDisplayElementWithContentDesc(
                _case_select[0]) is True, "Navigation drawer not displayed"
            ...
            assert self.dut.mbs.uiWatcherTextContains(
                "isn't responding") is False, "Crash found"
    ...
    def test_favourite_in_all_contact(self):
        ...
        try:
            ...
            time.sleep(1)
            assert self.dut.mbs.assertTextDisplayed(
                _selector[-1]) is True, "1 Selected text not found"
        finally:
            logging.info("Changing screen orientation")
    ...
    def test_share_contacts(self):
        ...
        time.sleep(8)
        assert self.verify_text_on_screen(
            image=_gmail_image, txt=_subject) is True, "Sent mail not found in inbox"
        ...
        assert self.verify_text_on_screen(
            image=_image_drive, txt=_selector[7]) is True, "Drive Verification Failed"
    ...
    def navigate_to_all_app_info(self):
        ...
        assert self.dut.mbs.waitUntillDisplayElementWithText(
            "Apps & notifications") is True, "Apps & notifications option not displayed in settings"
    ...
    def get_screen_text_if_assert_fail(self, fail_str, image_name):
        ...
        logging.info("Assert failed '%s' , capturing screen-shot %s", fail_str)
    ...
    def assert_pop_up_text(self, contact_name):
        ...
        assert self.dut.mbs.assertTextDisplayed(
            txt) is True, "Expected text not found on 'Delete' popup"
        assert self.dut.mbs.assertTextDisplayed(
            "Cancel") is True, "Expected text not found on 'Delete' popup"
        assert self.dut.mbs.assertTextDisplayed(
            "Delete") is True, "Expected text not found on 'Delete' popup"
    ...
    def delete_all_contacts(self):
        ...
        assert self.dut.mbs.waitUntillDisplayElementWithContentDesc(
            "More options") is True, "Three dot menu option not displayed"
    ...
    def set_contact_photo(self):
        ...
        assert self.dut.mbs.waitUntillDisplayElementWithContentDesc(
            "Add contact photo") is True, "Camera button not found on create contact screen"
        self.dut.mbs.waitAndClickUsingContentDesc("Add contact photo")
        assert self.dut.mbs.waitUntillDisplayElementWithText(
            "Take photo") is True, "Take photo popup not found"
        ...
        assert self.dut.mbs.waitUntillDisplayElementWithText(
            uiElement['CAMERA_RESET_DONE']) is True, "After click Reset Done screen not found"
    ...
    def get_screen_text_if_assert_fail(self, fail_str=None, image_name=None):
        """
        To take screenshot of the screen if assertion failed
        :param fail_str:
        :param image_name:
        :return:
        """
